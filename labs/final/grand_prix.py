...
### State Update ###
def update():
    global curState, arColorGlobal, speed
    """
...
def wallFollow():
    ...
    speed = rc_utils.remap_range(front_dist, 0, 250, 1, 2.1, True)
...
def update_contour_alt(colorList, image):
    """
    Finds contours in the current color image and uses them to update contour_center
    and contour_area
    """
...
def canyonLineFollowing(arColor):
    ...
    update_contour_alt([PURPLELINE, ORANGELINE], image)
    # if arColor == "PURPLE":
    ...
    if contour_center is not None:
        ...
        if arColor == "PURPLE" or arColor == "ORANGE":
            ...
            if rightLine % 2 == 0:
                ...
                centerInv = rc_utils.clamp(contour_center[1], int(0.5 * quarterImgWidth), quarterImgWidth)
                angle = rc_utils.remap_range(centerInv, int(0.5 * quarterImgWidth), quarterImgWidth, -1, 1)
                print("Angle: ", angle)
            if rightLine % 2 == 1:
                #LEFT FOLLOWING
                centerInv = rc_utils.clamp(contour_center[1], 0, int(0.5 * quarterImgWidth))
                angle = rc_utils.remap_range(centerInv, 0, int(0.5 * quarterImgWidth), -1, 1)
...
def newCanyon():#arColor):
    ...
    update_contour(ORANGELINE)
    if contour_center is None:
        update_contour(PURPLELINE)
